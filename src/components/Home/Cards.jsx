...
function Cards({ t }) {
    ...
    const { defaultPoints, defaultType, statusOfPoint } = useDefaultPoints();
    ...
    const idsDefaultPoints = ids.filter((id) => id.includes(`${statusOfPoint}`));
    ...
    useEffect(() => {
        if (idsDefaultPoints.length === 0) {
            defaultPoints.forEach((point) => dispatch(fetchDataOfWeather({ currentPoint: point, currentType: defaultType, statusOfPoint })))
        }
    }, [defaultPoints]);
